...
class NeuralNetwork(Module):
    ...
    _key_state: eqxe.StateIndex = static_field()
    ...
    def __call__(
        ...
        **Arguments**:
        ...
        - `key`: A `jax.random.PRNGKey` used for dropout. Optional, keyword-only
            argument. If `None`, a key will be generated by getting the key stored in
            the network and then splitting and updating the key for the next forward
            pass, ensuring a different key each time.
    ...
    def _get_key(self) -> jr.PRNGKey:
        """Get the random key stored in `self._key_state` (an `eqxe.StateIndex`)."""
        return eqxe.get_state(self._key_state, jr.PRNGKey(0))

    def _keygen(self) -> jr.PRNGKey:
        """Get the random key contained in `self._key_state` (an `eqxe.StateIndex`),
        split the key, set `self._key_state` to contain the new key, and return the
        original key.
        """
        key = self._get_key()
        _, new_key = jr.split(key)
        eqxe.set_state(self._key_state, new_key)
        return key
    ...
    def _set_parameters(
        ...
        self._key_state = eqxe.StateIndex()
        dkey, key = jr.split(key, 2)
        eqxe.set_state(self._key_state, dkey)
